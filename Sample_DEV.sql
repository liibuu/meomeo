...
alter table VIB_SAMPLE_DEV add flag_activity float

update VIB_SAMPLE_DEV set flag_activity = 1
...
where VIB_SAMPLE_DEV.CUSTOMER_NUMBER = a.CUSTOMER_NUMBER

--2.2 flag noise
alter table VIB_SAMPLE_DEV add flag_trans float, flag_only_acti_trans float,
							flag_noise float

update VIB_SAMPLE_DEV set flag_trans = 1
from	(select CUSTOMER_NUMBER from Data_MyVIB_Transaction
		where TRANS_DATE >= '2019/04/01' and TRANS_DATE <= '2019/06/30') a
where VIB_SAMPLE_DEV.CUSTOMER_NUMBER = a.CUSTOMER_NUMBER

update VIB_SAMPLE_DEV set flag_only_acti_trans = 0
from	(select CUSTOMER_NUMBER
		from Data_MyVIB_Activity
		where ACTIVITY_DATE >= '2019/04/01' and ACTIVITY_DATE <= '2019/06/30'
		and ACTIVITY_NAME not in (	'SET_PASSWORD',
...
update VIB_SAMPLE_DEV set flag_only_acti_trans = 1 where flag_only_acti_trans is null

update VIB_SAMPLE_DEV set flag_noise = case when flag_trans is null and flag_only_acti_trans = 1 then 1 else null end

--2.3 flag good bad
alter table VIB_SAMPLE_DEV add flag_bad float

update VIB_SAMPLE_DEV set flag_bad = case when flag_activity = 1 and flag_noise is null then 0 else 1 end 
