...
--into VIB_Tinhbien_Activity_OOT
into VIB_Tinhbien_Transaction_OOT
from (		select CUSTOMER_NUMBER, 1 as TIME_ID, flag_bad from VIB_SAMPLE_OOT
		union all
			select CUSTOMER_NUMBER, 2 as TIME_ID, flag_bad from VIB_SAMPLE_OOT
		union all
			select CUSTOMER_NUMBER, 3 as TIME_ID, flag_bad from VIB_SAMPLE_OOT
	) a

alter table VIB_Tinhbien_Activity_OOT 
...
alter table VIB_Tinhbien_Transaction_OOT 
...
update VIB_Tinhbien_Activity_OOT set	ACTIVITY_M_NO = isnull(c.ACTIVITY_M_NO,0),
...
from (select a.CUSTOMER_NUMBER, a.TIME_ID,	sum(b.ACTIVITY_NO)											as ACTIVITY_M_NO, 
		...
		from VIB_Tinhbien_Activity_OOT a
		...
		and b.ACTIVITY_DATE <= dateadd(month,cast(-1 as float)*a.TIME_ID + 1,'2019/09/30')
		and b.ACTIVITY_DATE > dateadd(month,cast(-1 as float)*a.TIME_ID,'2019/09/30')
		group by a.CUSTOMER_NUMBER, a.TIME_ID
		) c
where VIB_Tinhbien_Activity_OOT.CUSTOMER_NUMBER = c.CUSTOMER_NUMBER
and VIB_Tinhbien_Activity_OOT.TIME_ID = c.TIME_ID



---******bien Activity******---

update VIB_Tinhbien_Activity_OOT set	ACTIVITY_NO = isnull(c.ACTIVITY_NO,0),
...
update VIB_Tinhbien_Transaction_OOT set	TRANS_NO = isnull(c.TRANS_NO,0),
...
from (select a.CUSTOMER_NUMBER, a.TIME_ID,	sum(b.TRANS_NO)										as TRANS_NO, 
		...
		from VIB_Tinhbien_Transaction_OOT a
		left join Data_MyVIB_Transaction b
		on a.CUSTOMER_NUMBER = b.CUSTOMER_NUMBER
		and b.TRANS_DATE <= dateadd(month,cast(-1 as float)*a.TIME_ID + 1,'2019/09/30')
		and b.TRANS_DATE > dateadd(month,cast(-1 as float)*a.TIME_ID,'2019/09/30')
		group by a.CUSTOMER_NUMBER, a.TIME_ID
		) c
where VIB_Tinhbien_Transaction_OOT.CUSTOMER_NUMBER = c.CUSTOMER_NUMBER
and VIB_Tinhbien_Transaction_OOT.TIME_ID = c.TIME_ID
